...
- Compound Chinese unit `万亿` now uses longest-token matching and normalizes to 10^12 without hiding genuine missing-number findings.
...
- Schema-4 bilingual EPUB 回填改为单次 lxml 资源渲染：共享严格源文清洗、容器/直 `<br>` 配对与双语样式契约，支持 `target_first` 和 `source_first` 并在发布前拒绝映射与保留标记冲突。
...
- 修复 EPUB schema 4 槽位回填、双语源文配对、跨槽位标点规范化及脚注标记识别，并补齐严格流水线的断点续跑与审校统计。
