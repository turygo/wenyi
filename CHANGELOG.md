...
- 新增完全离线的基准语料库扫描、人工筛选、冻结、运行，以及评估产物生成和校验命令；所有产物均包含稳定哈希，工具还会检查跨书配额和数据泄漏，并拒绝覆盖已有输出目录。
- 新增第一层基准归因与金丝雀测试运行器：严格校验候选配置和冻结的准备数据，执行 `C0/C1/C2` 上下文诊断；同一份翻译产物可分发给润色分支和未润色对照分支，并支持原子化日志、断点续跑和生产润色质量门禁。
- 新增第六阶段的冻结准备数据导出、离线校验和完整基准测试命令：复用生产预处理状态，在正式连续语料上执行完整的质量 `DAG`；关闭 `raw` 分支的 `RunStore` 后，再安全地克隆出 `editor` 和 `fast` 分支，并拒绝各分支重复翻译。
- 新增第七阶段的离线人工评估包，并将模型信息盲化：采用六类严格定义的任务模式，支持确定性分配、校准和隐藏重复项，提供静态本地界面、只读的保密映射、严格的增量响应导入、哈希绑定和完成清单。
