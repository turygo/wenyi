...
ACTIONABLE_TYPES = frozenset({
    ISSUE_QUOTE_LOSS, ISSUE_NUMBER_MISMATCH, ISSUE_TERM_MISS,
    ISSUE_UNTRANSLATED, ISSUE_EMPTY,
})
...
_EN_MULT = {"thousand": 1000, "million": 1_000_000, "billion": 1_000_000_000,
            "grand": 1000, "decade": 10, "decades": 10}
...
def _number_mismatch_detail(src_values: set[float], tgt_values: set[float],
                            tgt_wildcard_prefixes: list[str]) -> str | None:
...
def _term_miss_details(source: str, target: str, locked_terms) -> list[str]:
    ...
    for term in locked_terms:
        ...
        if t_target in (target or ""):
            continue
        details.append(f"锁定术语「{t_source}」应译为「{t_target}」，译文未出现该译名，请核对后重译")
    return details
    ...
    *,
    ...
    for i, (s, t) in enumerate(zip(sources, targets)):
        ...
        nm_detail = _number_mismatch_detail(
            _extract_source_numbers(s, src_lang), _extract_target_numbers(t),
        ...
        if src_lang != "zh" and _is_untranslated(s, t):
            issues.append(LintIssue(
                i, ISSUE_UNTRANSLATED, "译文与原文高度重合，疑似整段未译，请重新翻译",
            ))
