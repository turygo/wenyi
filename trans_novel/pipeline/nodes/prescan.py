...
from trans_novel.pipeline.fingerprints import fast_model_profile, primary_model_profile
...
class DigestNode:
    ...
    def __init__(self, *, synopsizer, config: Config):
    ...
    def execute(self, request: NodeRequest) -> NodeOutcome:
        ...
        # 幂等双保险：梗概已落且指纹一致（planner 已排除，防御续跑路径）。
...
class MineTermsNode:
    ...
    def __init__(self, *, namer, config: Config):
        self.namer = namer
        self.config = config
...
class NameTermsNode:
    ...
    def __init__(self, *, namer, analyzer, glossary: GlossaryStore, config: Config):
        self.namer = namer
...
class BookSynopsisNode:
    ...
    def __init__(self, *, synopsizer, analyzer, glossary: GlossaryStore, config: Config):
        self.synopsizer = synopsizer
        self.analyzer = analyzer
        self.glossary = glossary
        self.config = config
