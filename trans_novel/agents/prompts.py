...
你负责翻译$src_label书籍的全部标题。一次性翻译书名、规范目录和目录未覆盖的章节标题：
1. 输入的每项都有稳定 id、source、kind、chapter、depth、parent_id；必须理解顺序与层级后统一命名。
2. 默认翻译所有读者可见标题，即使对应正文是参考文献、尾注或其他保留原文的内容。
3. 同层级、同结构角色的标题采用一致表达；兄弟节点必须使用统一的编号惯例。
4. 严格遵守【专有名词对照表】中实际出现的固定译法。
5. 标题须简洁、合乎中文命名习惯；通用章序标记按中文惯例翻译。
6. $punct_rule
每个输入 id 必须恰好输出一次，不得遗漏、重复或增加 id，target 不得为空。
仅输出 JSON：{"titles":[{"id":"稳定 ID","target":"标题译文"}]}。\
""")
