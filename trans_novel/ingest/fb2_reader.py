...
def _image_id(el: ET.Element) -> str:
    """返回 `<image>` 元素通过 `xlink:href` 或普通 `href` 引用的 `<binary>` ID。"""
...
def _direct_segments(
    ...
    避免诗歌、引文、小标题被丢字。记录图片相对于段落的位置，导出时再从原 FB2
    的 `<binary>` 元素恢复图片资源。返回 (标题文本, segments, images)；标题作为 heading 排首。
    """
...
def read_fb2_binaries(path: str) -> dict[str, tuple[str, bytes]]:
    """按需读取 FB2 的内嵌资源，避免将 `<binary>` 元素中的大段数据写入断点状态。"""
