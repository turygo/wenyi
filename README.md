...
多个职责单一的 Agent 协同，模拟出版社流程：**全书理解预扫 → 分析 → 翻译 → 审校 → 标点规范化 → 术语 AI 审计统一 → 跨章一致性把关**，全程围绕一个持久化的术语库。**一条 `translate` 命令连续跑完并直接产出 EPUB，或用 `web` 在浏览器里可视化操作。**
...
日常只需 `web` 或 `translate` 两个命令；细粒度/调试工具收敛到 `tools` 子命令。

```bash
# 推荐：可视化控制台（浏览器内选/上传书籍、运行、暂停/续跑、看双语流与术语表）
uv run trans-novel web                    # 不带文件！启动后打开 http://127.0.0.1:8000
...
```bash
uv run trans-novel web                    # 浏览器里选/上传文件，命令行不再指定
```

- **持久书架**：列出 `state_dir` 里所有书 + 进度 + 状态（**关浏览器/重启服务后仍在**，点开即续）。
- **浏览器内选/上传书籍**：本地选 `.epub/.fb2/.txt`，无需命令行指定路径。
- **运行 / 暂停 / 续跑**：勾选步骤（翻译/术语审计/QA/报告/回填，可单选可全选）→ 运行；**暂停**会在批边界优雅停下（已落盘），再点即从断点续跑。
- **关浏览器不中断**：任务跑在服务端后台；重连 = 读盘渲染已译内容 + WebSocket 接后续增量。
- **实时双语流**：进度条 + "原句↔译句"批次级对照；该段有问题挂角标。
- **建议/修订**：审校建议、回译疑点、一致性问题、术语统一，标注 ✓已修订 / ⚠仅建议。
- **前端直接编辑术语表**：改译法（保存即改写正文）、锁定、删除、裁决冲突、单条"应用到正文"、"重新应用术语表到正文"一键统一全书。

> 自动改写边界：**术语审计**会自动改写正文（消除译法漂移）；**一致性 QA / 报告**只汇总不改正文——需要的改动通过术语表编辑/重新应用来落地。
...
- **断点续跑（章/批级）**：每批译完即增量落盘，`resume` 跳过已完成的批，只补未完成的；
  Web 的"暂停"在批边界优雅停下，同样可续。
...
trans_novel/
  ...
  agents/      analyzer / synopsis(全书理解) / translator / reviewer / polisher / consistency / glossary_auditor + 提示词
  pipeline/    orchestrator(状态机/续跑/暂停) / context(滚动上下文) / checks(对齐校验) / runstore
  postprocess/ 标点规范化
  assemble/    回填(EPUB/TXT，书名/目录译名) + QA 报告
  web/         FastAPI 控制台（run 列表/上传/运行/暂停/WS）+ 原生单页前端
