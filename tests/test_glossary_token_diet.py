...
def _cfg(state_dir, *, back_matter="full", max_chars_per_batch=1800,
    ...
    return Config.from_dict({
        ...
        "segment": {"max_chars_per_batch": max_chars_per_batch,
                    "max_chars_per_segment": 1200},
        "pipeline": {"review": review, "polish": polish,
...
class TestBackMatterFullNoExtraction(unittest.TestCase):
    ...
    def test_full_backmatter_runs_pipeline_without_extraction(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            body_ci = next(c["index"] for c in m["chapters"]
                           if not is_back_matter(c["title"]))
            ...
            self.assertTrue(
                {"batch_translated", "batch_polished", "chapter_reviewed",
                 "chapter_backtranslation_checked"}.issubset(bm_ev),
                f"full 档附属章应走完整流水线，实际事件={sorted(bm_ev)}")
            # 但零抽词事件（所有抽取点被 not bm 守住）
            self.assertNotIn("batch_glossary_extracted", bm_ev,
                             "full 档附属章不得批内抽词")
            self.assertNotIn("chapter_glossary_extracted", bm_ev,
                             "full 档附属章不得章末兜底抽词")
            # 正文章不受影响，仍抽词
...
class TestBatchExtractionPruning(unittest.TestCase):
    ...
    def test_inline_extraction_existing_pruned_to_batch(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            txt = os.path.join(d, "novel.txt")
            _write(txt, doc)
            cfg = _cfg(os.path.join(d, "state"), max_chars_per_batch=8,
                       review=False, polish=False, backtranslate=0.0,
                       book_understanding=False, consistency_qa=False)
...
class TestBatchSnapshotFreeze(unittest.TestCase):
    ...
    def _run(self, doc: str, new_source: str, new_target: str) -> dict[str, str]:
            ...
            if "术语" in system and "抽取器" in system:
                ...
                if new_source in user:
                    return json.dumps({"terms": [
                        {"source": new_source, "target": new_target, "type": "称谓"}
                    ]}, ensure_ascii=False)
                return json.dumps({"terms": []}, ensure_ascii=False)
            return routing_handler(messages, tier, json_mode)
        ...
        with tempfile.TemporaryDirectory() as d:
            txt = os.path.join(d, "novel.txt")
            _write(txt, doc)
            cfg = _cfg(os.path.join(d, "state"), max_chars_per_batch=6,
                       review=False, polish=False, backtranslate=0.0,
                       book_understanding=False, consistency_qa=False)
