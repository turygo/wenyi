...
from trans_novel.glossary.miner import Candidate, mine_candidates_en
...
class TestMineCandidatesDualChannel(unittest.TestCase):
    """TermMiner：en 走确定性大写通道与 LLM 通道合并。"""

    @staticmethod
    def _miner(src: str, candidates: list[str]) -> TermMiner:
        ...
        return TermMiner(client, config)
    ...
    def test_en_merges_llm_lowercase_terms_missed_by_uppercase_channel(self):
        ...
        result = self._miner("en", ["lithography", "Morris Chang"]).mine([(1, text)])
...
class TestMineCandidatesLlmConcurrency(unittest.TestCase):
    ...
    def _mine(self, concurrency: int, on_progress=None, per=None):
        per = self._PER if per is None else per

        def handler(messages, agent, operation, json_mode):
            ci = int(re.search(r"第(\d+)章", messages[1]["content"]).group(1))
            value = per[ci]
            if isinstance(value, Exception):
                raise value
            return json.dumps({"candidates": value})

        config = Config.from_dict({"llm": fake_llm_dict()})
        config.source_lang = "ja"
        config.target_lang = "zh"
        miner = TermMiner(FakeClient(handler=handler), config)
        return miner.mine(self._CHAPTERS, concurrency=concurrency, on_progress=on_progress)
    ...
    def test_concurrent_output_identical_to_serial(self):
        ...
        self.assertEqual(by_surface["堀北"].chapters, [1, 2, 3])
