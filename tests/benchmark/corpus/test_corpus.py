...
from trans_novel.benchmark.corpus import CorpusError, build, scan, validate_corpus
from trans_novel.benchmark.corpus.identity import count_words, passage_id, segment_id, source_digest
...
def _write_round_trip_specs(
    root: Path, books: list[dict[str, str]], passages: list[dict]
) -> tuple[Path, Path]:
...
def _round_trip_fixture(
    root: Path,
) -> tuple[Path, Path, dict[str, Document], list[dict]]:
    ...
    for index in range(10):
        ...
        if split == "screen":
            ...
            segments = [Segment(index=0, source=("continuousword " * 10000).strip())]
            passages.append(
                {
                    "subset": "continuous",
                    "book_id": book_id,
                ...
                passages.append(
                    {
                        ...
                        "context": {
                            "challenge_type": "polysemy",
                            "source_before": [{"chapter_index": 0, "segment_index": before_index}],
                            "frozen_target_before": [
                                {
                                    "chapter_index": 0,
        ...
        books.append({"book_id": book_id, "path": source_path.name, "split": split})
    spec, selection = _write_round_trip_specs(root, books, passages)
    return spec, selection, documents, passages
...
class CorpusPrimitiveTests(unittest.TestCase):
    ...
    def test_build_validate_round_trip_and_cli_breakdown(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            spec, selection, documents, passages = _round_trip_fixture(root)
            out = root / "corpus"
            with patch(
    ...
    def test_scan_is_canonical_and_has_deterministic_tags(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(records[0]["kind"], "heading")
