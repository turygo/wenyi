    ...
    cfg = Config.from_dict(
        {
            "llm": {
                "provider": "fake",
                "models": {"primary": "primary-id", "fast": "fast-id"},
            }
        }
    )
...
class TestAgentRouter(unittest.TestCase):
    def test_primary_agents_use_primary_model_with_reasoning(self):
        for agent in ("translator", "editor", "analyst"):
    ...
    def test_explicit_suffix_overrides_role_defaults(self):
        ...
        primary, fast = (call["model_ref"] for call in transport.calls)
    ...
    def test_all_production_agents_are_routable(self):
        transport = StubTransport()
        router = _router(transport)
        for agent in PRODUCTION_AGENT_IDS:
            router.complete([{"role": "user", "content": "x"}], agent=agent, operation="op")
        self.assertEqual(len(transport.calls), len(PRODUCTION_AGENT_IDS))
